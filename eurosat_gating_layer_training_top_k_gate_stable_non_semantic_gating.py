...
class StrictGatedCNN(nn.Module):
    ...
    def forward(self, x, labels=None, path_name=None):
        ...
        if self.training:
            ...
            gate_weights = F.softmax(raw_scores / 0.1, dim=1)
...
def evaluate_global_per_expert(global_model, loader, device):
    ...
    for path_name in ["path1", "path2", "path3"]:
        ...
        for X_batch, y_batch in loader:
            ...
            log_probs, _, _2 = global_model(X_batch, y_batch, path_name=path_name)
...
def evaluate_global_by_label_paths(model, loader, device):
    ...
    for X_batch, y_batch in loader:
        ...
        for path_name, (lo, hi) in [
            ...
            log_probs, _, _2 = model(X_sub, y_sub, path_name=path_name)
    ...
    for rnd in range(num_rounds):
        ...
        for client_name, client_info in path_clients.items():
            ...
            for path_idx, expert_name in enumerate(EXPERT_NAMES):  # ["path1","path2","path3"]
                ...
                # for layer_name in ['conv8', 'conv9', 'conv10']:
                ...
                for epoch in range(LOCAL_FL_EPOCHS):
                    for X_batch, y_batch in loader:
                        ...
                        logits, gateweights, _ = client_model(X_batch, y_batch)  # pathname=None
                        chosen = gateweights.argmax(dim=1)

                        if expert_name == 'path1':
                            label_mask = (y_batch >= 0) & (y_batch <= 2)
                            gate_mask  = (chosen == 0)
                        elif expert_name == 'path2':
                            label_mask = (y_batch >= 3) & (y_batch <= 5)
                            gate_mask  = (chosen == 1)
                        else:  # path3
                            label_mask = (y_batch >= 6) & (y_batch <= 9)
                            gate_mask  = (chosen == 2)

                        # mask = label_mask & gate_mask
                        mask = label_mask
                        if not mask.any():
                            continue
                        ...
                        ysub = y_batch[mask]
                        optimizer.zero_grad()
                        logprobs, _ , _2 = client_model(Xsub, ysub, path_name=expert_name)
                        loss = criterion(logprobs, ysub)
                        loss.backward()
                        optimizer.step()
        ...
        for client_name, client_info in path_clients.items():
            ...
            for epoch in range(LOCAL_FL_EPOCHS):
                for X_batch, y_batch in loader:
                    ...
                    loss = 1.0 * loss_gate + 2.0 * loss_bal
                    loss.backward()
                    optimizer.step()
            ...
            # ]
            keys_to_avg = [k for k in model.state_dict().keys() if "gate_fc" in k]
            num_elements = 0
            for key in keys_to_avg:
                tensor = cs[key].clone()
...
def server_side_gate_pretraining(model, train_loader, device, num_epochs=15, lam_gate=1.0):
    model.to(device)
    model.train()
    ...
    # # Reduce gate learning rate from 1e-2 to 1e-3 for stability
    # gate_params = [p for n, p in model.named_parameters() if 'gate' in n and p.requires_grad]
    # head_params = [p for n, p in model.named_parameters() if n.startswith('fc') and p.requires_grad]
    
    # optimizer = torch.optim.Adam([
    #     {'params': gate_params, 'lr': 1e-3}, # Lowered LR
    ...
    # 2. OPTIMIZER: ONLY GATE PARAMS
    # Filter only parameters that require grad (this will now be gate-only)
    gate_params = [p for p in model.parameters() if p.requires_grad]
    
    optimizer = torch.optim.Adam(gate_params, lr=1e-3) # Use a stable LR
    ...
    for epoch in range(num_epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            # Get target paths for the classes
            target_paths = class_to_path_target(y_batch, device)
            target_indices = torch.argmax(target_paths, dim=1)

            logits, gate_weights, raw_scores = model(X_batch)

            # loss = criterion_gate(raw_scores, target_indices)
            
            # # Add diversity penalty (Load Balancing)
            # loss_cls = criterion_gate(raw_scores, target_indices)
            # # Penalize the gate if the mean probability for any expert is too low
            # probs = F.softmax(raw_scores, dim=1)
            # importance = probs.mean(dim=0)
            # loss_bal = torch.sum((importance - 1.0/3.0)**2) # Target 1/3 for each path
            # # Final loss (increase lam_gate if collapse persists)
            # loss = loss_cls + 1.0 * loss_bal 
            ...
            # Negative entropy forces the gate to be "surprised" and use more paths
            loss = loss_cls - 0.1 * entropy 
        ...
        print(f"Epoch {epoch+1} Gate Pretrain Loss: {avg_loss:.4f}")

        # CHECKPOINT: Save the best state
        if avg_loss < best_loss:
            best_loss = avg_loss
            best_weights = copy.deepcopy(model.state_dict())
            print(f"--> New best gating found at Epoch {epoch+1}")

    # Restore the best found gating weights
    if best_weights:
        model.load_state_dict(best_weights)
        print(f"Restored best gating model (Loss: {best_loss:.4f})")
...
for client_name, client_info in path_clients.items():
    ...
    # Inside the client loop, before training starts:
    # 1. Freeze EVERYTHING first
    for param in model.parameters():
        param.requires_grad = False

    # 2. Unfreeze ONLY convolutional layers
    for name, param in model.named_parameters():
        if "conv" in name:
            param.requires_grad = True

    # 3. Create optimizer with ONLY the unfrozen conv parameters
    optimizer = torch.optim.Adam(
    ...
    for epoch in range(NUM_LOCAL_EPOCHS):
        ...
        for X_batch, y_batch in loader:
            ...
            # 1. GLOBAL PASS (No path_name) to get gate scores for the whole batch
            _, _, rawscores_global = model(X_batch) 
            ...
            # Calculate gate loss once for the whole batch
            loss_gate = F.cross_entropy(rawscores_global, target_path_indices)

            total_loss_cls = 0
            for p_name, (low, high) in [('path1', (0,2)), ('path2', (3,5)), ('path3', (6,9))]:
                mask = (y_batch >= low) & (y_batch <= high)
                if mask.any():
                    logits, _, _ = model(X_batch[mask], y_batch[mask], path_name=p_name)
                    total_loss_cls += criterion(logits, y_batch[mask])

            # 4. BACKWARD: loss_gate is included, but only 'conv' weights will update
            loss = total_loss_cls + 0.1 * loss_gate
            loss.backward()
            optimizer.step()
